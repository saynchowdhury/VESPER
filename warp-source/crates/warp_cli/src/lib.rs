...
pub fn binary_name() -> Option<String> {
    ...
    let name = Path::new(&arg0).file_name()?.to_str().map(|s| s.to_owned());
    
    // If we're running in dev/test, it might be 'cargo' or some temp path, 
    // so we provide a sane fallback for branding.
    match name {
        Some(ref s) if s == "warp" || s == "warp-oss" => Some("vesper".to_string()),
        _ => name,
    }
}
