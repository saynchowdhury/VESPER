...
// Vesper white-label change — safe, no UI impact
        let app_id = AppId::new("dev", "vesper", "Vesper");
        Self {
            channel,
...
// Vesper white-label change — safe, no UI impact
    pub fn url_scheme() -> &'static str {
        match Self::channel() {
            Channel::Stable => "vesper",
            Channel::Preview => "vesperpreview",
            Channel::Dev => "vesperdev",
            // Dummy value--integration tests shouldn't support URL schemes.
            Channel::Integration => "vesperintegration",
            Channel::Local => "vesperlocal",
            Channel::Oss => "vesper",
        }
    }
}
