...
pub const WARP_CONFIG_DIR: &str = ".vesper";

/// The name of the folder that stores Vesper execution logs and network logs.
    ...
    cfg_if::cfg_if! {
        if #[cfg(target_os = "linux")] {
            ...
            let base_app_name = match app_id.application_name() {
                ...
                // Vesper white-label change — safe, no UI impact
                                "Vesper" => "Vesper".to_owned(),
                                other if other.starts_with("Vesper") => other.replace("Vesper", "Vesper-"),
