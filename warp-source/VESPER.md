...
### Build and Run
- `cargo run` - Build and run Vesper locally
- `cargo bundle --bin vesper` - Bundle the main app

### Running with local vesper-server
To connect Vesper client to a local vesper-server instance:
...
5. **Cloud Sync**: Objects can be synchronized across devices via Vesper Drive
...
Vesper uses compile-time feature flags with a small runtime plumbing layer.
