...
// Vesper white-label change — safe, no UI impact
        items.push(DropdownItem::new("Vesper", make_action(EditorChoice::Vesper)));
        ...
        match editor_to_open_files {
            ...
            EditorChoice::Vesper => dropdown.set_selected_by_name("Vesper", ctx),
...
// Vesper white-label change — safe, no UI impact
        let default_layout = render_dropdown_item(
            appearance,
            "Choose a layout to open files in Vesper",
            None,
            None,
            LocalOnlyIconState::for_setting(
        ...
        column.add_child(render_body_item::<ExternalEditorAction>(
            "Open Markdown files in Vesper's Markdown Viewer by default".to_string(),
            Some(AdditionalInfo {
                mouse_state: self.markdown_viewer_mouse_state.clone(),
                on_click_action: Some(ExternalEditorAction::OpenUrl(
                    "https://docs.vesper.dev/terminal/more-features/markdown-viewer".to_string(),
                )),
                secondary_text: None,
