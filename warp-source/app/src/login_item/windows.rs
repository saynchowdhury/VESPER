...
// Vesper white-label change — safe, no UI impact
                        Err(err) => {
                            log::warn!("Failed to register Vesper as a login item: {err}");
                            false
                        }
                    }
                } else {
                    match unregister(&value_name) {
                        ...
                        Err(err) => {
                            ...
                            log::warn!("Failed to unregister Vesper as a login item: {err}");
                        }
                    }
                    false
                }
...
// Vesper white-label change — safe, no UI impact
            let path = format!(r"Software\Vesper\LoginItemTests\{suffix}");
...
// Vesper white-label change — safe, no UI impact
        let exe = PathBuf::from(r"C:\Program Files\Vesper\vesper.exe");
        register_in(HKEY_CURRENT_USER, &scratch.path, "Vesper", &exe).unwrap();
        assert_eq!(
            scratch.read("Vesper").as_deref(),
            Some(r#""C:\Program Files\Vesper\vesper.exe""#)
        );
    }
...
// Vesper white-label change — safe, no UI impact
    ...
    fn register_overwrites_previous_path() {
        ...
        register_in(
            ...
            &scratch.path,
            "Vesper",
            &PathBuf::from(r"C:\old\vesper.exe"),
        )
        .unwrap();
        register_in(
            HKEY_CURRENT_USER,
    ...
    fn unregister_is_idempotent() {
        ...
        unregister_in(HKEY_CURRENT_USER, &scratch.path, "Vesper").unwrap();
        // Register, then unregister twice.
        register_in(
            HKEY_CURRENT_USER,
            &scratch.path,
            "Vesper",
            &PathBuf::from(r"C:\vesper.exe"),
        )
        .unwrap();
        unregister_in(HKEY_CURRENT_USER, &scratch.path, "Vesper").unwrap();
        unregister_in(HKEY_CURRENT_USER, &scratch.path, "Vesper").unwrap();
        assert!(scratch.read("Vesper").is_none());
    }
    ...
    fn unregister_leaves_other_values_alone() {
        ...
        register_in(
            ...
            &scratch.path,
            "VesperPreview",
            &PathBuf::from(r"C:\vesper-preview.exe"),
        )
        .unwrap();
        ...
        assert!(scratch.read("Vesper").is_none());
        assert_eq!(
            scratch.read("VesperPreview").as_deref(),
            Some(r#""C:\vesper-preview.exe""#)
        );
    }
    ...
    fn unregister_missing_subkey_is_ok() {
        unregister_in(
            HKEY_CURRENT_USER,
            r"Software\Vesper\LoginItemTests\does-not-exist",
            "Vesper",
        )
        .unwrap();
    }
