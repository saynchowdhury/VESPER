...
pub struct ServerApi {
    ...
    vesper_router: Arc<ai::VesperSmartRouter>,
...
impl ServerApi {
    fn new(
        ...
        // Vesper white-label change — safe, no UI impact
        // Initialize with default accounts using environment variables
        ...
        let accounts = vec![
            ...
            ai::InferenceAccount {
                id: "groq-2".to_owned(),
                provider: "groq".to_owned(),
                api_key: std::env::var("VESPER_GROQ_KEY_2").unwrap_or_default(),
                base_url: "https://api.groq.com/openai/v1".to_owned(),
                model: "llama3-70b-8192".to_owned(),
                daily_limit: 100000,
            },
            ai::InferenceAccount {
                id: "groq-3".to_owned(),
                provider: "groq".to_owned(),
                api_key: std::env::var("VESPER_GROQ_KEY_3").unwrap_or_default(),
                base_url: "https://api.groq.com/openai/v1".to_owned(),
                model: "llama3-70b-8192".to_owned(),
                daily_limit: 100000,
            },
        ...
        Self {
            ...
            #[cfg(feature = "agent_mode_evals")]
            eval_user_id,
            vesper_router: Arc::new(ai::VesperSmartRouter::new(accounts, http_client, persistence_sender.clone())),
            persistence_sender,
        }
    }
    ...
    {
        // Vesper white-label change — safe, no UI impact
        // Intercept Vesper models and route them through our local smart router
        if let Some(ref metadata) = request.metadata {
            if metadata.model_id == "vesper-fast" || metadata.model_id == "vesper-pro" {
                let model = if metadata.model_id == "vesper-pro" {
                    ai::VesperModel::VesperPro
                } else {
                    ai::VesperModel::VesperFast
                };
                
                // For simplicity in this white-label version, we'll assume Free tier unless we
                // implement the billing check logic here.
                let user_tier = ai::UserTier::Free;
                
                match self.vesper_router.complete_stream(request, model, user_tier).await {
                    Ok(stream) => {
                        let mapped_stream = stream.map(|res| {
                            res.map_err(|e| Arc::new(AIApiError::Other(anyhow::anyhow!(e))))
                        });
                        return Ok(mapped_stream.boxed());
                    }
                    Err(e) => return Err(Arc::new(AIApiError::Other(e))),
                }
            }
        }
...
impl ServerApiProvider {
    ...
    pub fn new(
        ...
        let mut server_api = ServerApi::new(auth_state.clone(), event_sender, agent_source, persistence_sender);
