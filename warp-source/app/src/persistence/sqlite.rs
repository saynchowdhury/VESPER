...
fn handle_model_event(event: ModelEvent, connection: &mut SqliteConnection) -> anyhow::Result<()> {
    match event {
        ...
        ModelEvent::UpsertVesperUsage { provider, tokens } => {
            upsert_vesper_usage(connection, &provider, tokens).context("error upserting vesper usage")
        }
    }
}

// Vesper white-label change — safe, no UI impact
fn upsert_vesper_usage(
    connection: &mut SqliteConnection,
    provider: &str,
    tokens: i64,
) -> Result<()> {
    use crate::persistence::schema::vesper_usage::dsl as usage_dsl;
    use crate::persistence::model::{VesperUsage, NewVesperUsage};

    let now = Utc::now().naive_utc();
    let reset_interval = chrono::Duration::days(1);

    let current: Option<VesperUsage> = usage_dsl::vesper_usage
        .filter(usage_dsl::provider.eq(provider))
        .first(connection)
        .optional()?;

    match current {
        Some(usage) => {
            if now - usage.last_reset_at > reset_interval {
                diesel::update(usage_dsl::vesper_usage.filter(usage_dsl::provider.eq(provider)))
                    .set((
                        usage_dsl::daily_tokens_used.eq(tokens),
                        usage_dsl::last_reset_at.eq(now),
                    ))
                    .execute(connection)?;
            } else {
                diesel::update(usage_dsl::vesper_usage.filter(usage_dsl::provider.eq(provider)))
                    .set(usage_dsl::daily_tokens_used.eq(usage_dsl::daily_tokens_used + tokens))
                    .execute(connection)?;
            }
        }
        None => {
            diesel::insert_into(usage_dsl::vesper_usage)
                .values(&NewVesperUsage {
                    provider,
