...
pub(crate) fn home_config_file_path(provider: MCPProvider) -> Option<PathBuf> {
    match provider {
        MCPProvider::Vesper => warp_core::paths::warp_home_mcp_config_file_path(),
...
// Vesper white-label change — safe, no UI impact
    ...
    // Vesper white-label change — safe, no UI impact
        ...
        pub fn project_config_path(&self) -> &'static Path {
            match self {
                MCPProvider::Vesper => Path::new(".vesper/.mcp.json"),
...
// Vesper white-label change — safe, no UI impact
    #[test]
    fn mcp_provider_from_file_path_recognizes_vesper_home_path() {
