...
// Vesper white-label change — safe, no UI impact
    pub fn icon(&self) -> Option<Icon> {
        match self {
            ...
            LLMProvider::Vesper => Some(Icon::Vesper),
...
// Vesper white-label change — safe, no UI impact
fn vesper_llm_infos() -> Vec<LLMInfo> {
    vec![
        LLMInfo {
            display_name: "Vesper Fast".to_owned(),
            base_model_name: "Vesper Fast".to_owned(),
            id: "vesper-fast".to_owned().into(),
            reasoning_level: None,
            usage_metadata: LLMUsageMetadata {
                request_multiplier: 1,
                credit_multiplier: None,
            },
            description: Some("Powered by Groq".to_owned()),
            disable_reason: None,
            vision_supported: true,
            spec: None,
            provider: LLMProvider::Vesper,
            host_configs: HashMap::new(),
            discount_percentage: None,
        },
        LLMInfo {
            display_name: "Vesper Pro".to_owned(),
            base_model_name: "Vesper Pro".to_owned(),
            id: "vesper-pro".to_owned().into(),
            reasoning_level: Some("High".to_owned()),
            usage_metadata: LLMUsageMetadata {
                request_multiplier: 1,
                credit_multiplier: None,
            },
            description: Some("Powered by Cloudflare".to_owned()),
            disable_reason: None,
            vision_supported: true,
            spec: None,
            provider: LLMProvider::Vesper,
...
impl Default for ModelsByFeature {
    fn default() -> Self {
        let mut agent_choices = vec![LLMInfo {
        ...
        agent_choices.extend(vesper_llm_infos());

        let mut coding_choices = vec![LLMInfo {
        ...
        coding_choices.extend(vesper_llm_infos());

        Self {
            agent_mode: AvailableLLMs {
                default_id: "vesper-fast".to_owned().into(),
...
// Vesper white-label change — safe, no UI impact
    pub fn new_choices_since_last_update(&self) -> Option<Vec<LLMInfo>> {
        self.last_update.as_ref().map(|update| {
            // We don't want to display new choices if they are branded as the original application.
            let filter_choices: Vec<LLMInfo> = update
                .new_choices
                .clone()
                .into_iter()
                .filter(|choice| !choice.display_name.to_lowercase().contains("warp"))
                .collect();
