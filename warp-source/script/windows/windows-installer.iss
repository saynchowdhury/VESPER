...
#define MyAppURL "https://www.vesper.local/"
#ifndef MyAppName
  #define MyAppName "VesperDev"
#endif
#ifndef MyAppVersion
...
#define ChannelPascalCase \
  ...
  "Unknown")))))
#define AppMutexName "Local\Vesper" + ChannelPascalCase + "_SingleInstance"


[Setup]
; NOTE: The value of AppId uniquely identifies this application. Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
AppId=vesper-terminal-{#ReleaseChannel}
AppName={#MyAppName}
...
WizardStyle=modern
WizardSmallImageFile="installer-images\vesper-logo.bmp"
WizardImageFile="installer-images\vesper-banner.bmp"
SetupIconFile="..\..\app\channels\{#ReleaseChannel}\icon\no-padding\icon.ico"
UninstallDisplayIcon="{app}\icon.ico"
; Force close previous Vesper if it hasn't shut down yet.
; In the update flow we already warn the user if they have something running and make them confirm
; before running this installer. Therefore, we are good to force close Vesper without fear of losing
; unsaved work.
...
; For manual installs: if Vesper is running, show a dialog prompting the user to close it
...
[Registry]
Root: HKCU; Subkey: "SOFTWARE\Vesper.local\{#MyAppName}"; Flags: uninsdeletekey
; cleanup "Open Vesper Here" registry entries
...
; Add "Open Vesper in new tab" to directory context menu
...
; Add "Open Vesper in new tab" to directory background context menu
...
; Add "Open Vesper in new window" to directory context menu
...
; Add "Open Vesper in new window" to directory background context menu
...
[Tasks]
Name: addToPath; Description: "Add Vesper to PATH"

[UninstallDelete]
Type: filesandordirs; Name: "{userappdata}\vesper\{#MyAppName}"
Type: filesandordirs; Name: "{localappdata}\vesper\{#MyAppName}"
Type: filesandordirs; Name: "{app}\bin"

[Icons]
Name: "{autoprograms}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\icon.ico"; AppUserModelID: "dev.vesper.{#MyAppName}"
Name: "{autodesktop}\{#MyAppName}"; Filename: "{app}\{#MyAppExeName}"; IconFilename: "{app}\icon.ico"; AppUserModelID: "dev.vesper.{#MyAppName}"; Tasks: desktopicon
...
{ Returns true when the installer was launched by Vesper's auto-update code.
  The auto-update path passes /update=1 on the command line and /NOCLOSEAPPLICATIONS
  so that the installer does not forcibly kill the running Vesper process. Instead we
  wait for Vesper to exit naturally by polling the app mutex below. }
...
{ For background updates, return an empty mutex name so that Inno Setup skips its
  ...
  installs, return the real mutex name so the user is prompted to close Vesper first. }
...
var
  ...
  ResultCode: Integer;
begin
  { Background update: the installer was launched while Vesper was still running.
    We passed /NOCLOSEAPPLICATIONS so Inno won't kill it. Wait here - before any
    files are touched - for Vesper to release its single-instance mutex, which
    happens as part of normal process exit. }
  if CurStep = ssInstall then
  begin
    if IsBackgroundUpdate() then
    begin
      Log('Background update: waiting for Vesper to exit (mutex: {#AppMutexName})...');
      WaitCounter := 0;
      ...
      begin
        Log('Vesper mutex still held after timeout; force-killing remaining processes.');
        { Kill by image name. {#MyAppExeName} (e.g. vesper.exe, dev.exe) is unique
          ...
          rely on Vesper's Job Object (JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE) to
          cascade-kill any child OpenConsole.exe processes when vesper.exe dies. }
        ...
        Log('Vesper has exited; proceeding with file installation.');
    end;
  end;

  { After a successful install, write a helper script for running the Vesper CLI. }
  { We use this to add a "vesper-" prefix (e.g. "vesper-preview.cmd" vs. "preview.exe") }
  if CurStep = ssPostInstall then begin
    { Add Vesper to PATH if requested }
