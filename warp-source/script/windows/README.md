...
a configuration file for building a Vesper installer.
...
* Run `cargo build` to ensure the installer uses the latest version of Vesper.
...
```shell
.\script\windows\Output\Vesper-Windows-Setup.exe`.
```
...
* `MyAppExeName` (default: `vesper.exe`)
